...
def rodar_teste_completo():
    ...
    # Eleitor 1 (1S Lima - 6012341) vota por clique
    res_voto1 = client.post("/api/fase4/votar", json={
    ...
    print(f"[OK] Eleitor 1 votou por clique. Comprovante: {res_voto1.json()['comprovante_hash']}")

    # Tentativa de Voto Duplo (bloqueio garantido)
    res_duplo = client.post("/api/fase4/votar", json={
        "identificador": "6012341",
        "votos": {"Graduados": cand_grad_depl, "Pracas": cand_praca_deco, "Civil": cand_civil_depj}
    })
    assert res_duplo.status_code == 400
    print("[OK] Bloqueio de voto duplicado funcionando perfeitamente.")

    # Eleitor 2 (2S Santos - 6012342) vota por clique
    client.post("/api/fase4/votar", json={
        "identificador": "6012342",
        "votos": {"Graduados": cand_grad_depl, "Pracas": cand_praca_deco, "Civil": cand_civil_depj}
    })

    # Eleitor 3 (Civil - 111.222.333-44) vota
    client.post("/api/fase4/votar", json={
